        ...
        <Row className="align-items-centers">
          ...
          <Col lg="6" xl={{ size: 5, offset: 1 }} className="align-self-center mt-4 mt-lg-0">
            {result && result.result === "error" &&
              <Alert color="danger">
                <FaTimes className="me-2" /> <span dangerouslySetInnerHTML={{ __html: result.msg }}></span>
              </Alert>}

            {result && result.result === "success" &&
              <Alert color="success">
                <FaCheck className="me-2" /> {result.msg}
              </Alert>}
            ...
            <Form inline onSubmit={this._handleSubmit} disabled={isLoading}>
              <FormGroup className="input-group mb-2">
                ...
                <Button size="lg" color="primary" className="text-white" disabled={isLoading}>Subscribe</Button>
              </FormGroup>
